...
void DesplayMarkSegment(void)
{
    ...
    int count=0;
    int choice;
    printf("请选择查询对象：1、及格 2、不及格");
    scanf("%d",&choice);
    switch(choice)
    {
    	case 1:
    		p=headLink->next;
            printf("60分以上(及格)的学生成绩如下：\n");
            printf("学号\t姓名\t成绩1\t成绩2\t成绩3\t总成绩\t平均成绩\n\n");
            while(p)
            {
            if((60<=((int)(p->mark1)))&&(60<=((int)(p->mark2)))&&(60<=((int)(p->mark3))))//每一科都及格才认定为及格 
            {
            count++;
            DesplayOneNode(p);
            }
